...
- `ReleaseGateMatchesCiGateTest` 에 검사 **둘**. 둘 다 리뷰가 낳았고 둘 다 **산문을 불변식으로 바꾼다**.
  ...
  | `everyTestTagIsGated` | **네 번째 `@Tag` 가 게이트 없이 생기는 것** — 세 파일이 산문으로 "opt-in 계층은 없다" 고 주장하는데 아무것도 그것을 지키지 않았다 | 테스트에 `@Tag("smoke")` 를 넣고 실패를 확인. **두 표기(`@Tag` · `@org.junit.jupiter.api.Tag`) 모두**로 확인했다 — 첫 초안은 정규식이 정규화되지 않은 앞쪽만 봐서 수식된 형태를 **놓쳤고**, 그 자체가 §0.4-a 의 함정이 세 번째로 나타난 것이다 |
...
- **낡아 있던 산문 여섯** — 처음에는 "셋", 1차 리뷰 뒤 "다섯" 으로 적었고 **둘 다 틀렸다.** 세 번째로
  ...
  | [`spring-boot-starter-open-items.md`](spring-boot-starter-open-items.md) B-5 | 다른 문서의 **닫힌 항목**이 2026-08 시점 근거를 인용한 것이다. 문장은 이제 낡았지만 그 항목이 주장하는 것(데몬 스레드)은 그대로 참이라 손대지 않았다 — 다만 여기 적어 두어야 "여섯" 이 다시 일곱이 되지 않는다 |
  ...
  *"the one tier still running / nowhere is `playwrightTest`"* 로 **두 줄에 걸쳐** 있어서 어느 한 줄도
  ...
  IMPORTANT: 세 번의 오차가 전부 **같은 종류**다. "셋" 은 커밋 하나가 고친 자리의 수였고, "다섯" 은
  **이 작업이 직접 고친 파일은 뺀다**는 무의식적 기준이 만든 수였다 — 그런데 그 기준은 성립하지 않는다:
  1·4·6번 모두 이 작업이 직접 고친 파일이고 1번은 `한 것` 불릿에도 중복으로 실려 있다. 규칙 일곱이
  *"분류는 표가 아니라 항목 본문에서 읽는다"* 고 말하는 것의 한 판본이며, 여기서 빠져 있던 것은 읽을
  본문이 아니라 **세는 기준을 글로 적는 일**이었다.
