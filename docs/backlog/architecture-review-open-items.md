...
|---|---|---|
| cold | 57초 | 94초 |
| warm | 6초 | 14~28초 (`--rerun-tasks`, 컴파일 포함. 스위트 자체는 3초 미만) |

어느 쪽도 한 자리 배수 안이다. 그러므로 **분리의 근거를 갈아 끼운다** — 비용이 아니라 같은 표의
...
#### 인구조사 — 문자열은 **세는 시점마다 달랐다** (5 → 6 → 5)

§0.4-a 의 함정이 그대로 있는데, 이번에는 한 번 더 접혔다. 범위는 `modules/**/*.java`, `Tag("playwright")`
문자열 검색이다.

| 세는 법 | 2026-08-31 (§0.4-a) | 착수 직전 (`53649d0`) | **이 커밋 이후** |
|---|---|---|---|
| `Tag("playwright")` 문자열 | 5 | **6** | **5** |
| 그중 애노테이션 | 4 | 4 | 4 |
| 애노테이션이 있는 클래스 | 1 | 1 | 1 |

**애노테이션과 클래스는 한 번도 움직이지 않았다.** 움직인 것은 문자열 수뿐이고, 그 등락은 둘 다 코드가
아니라 **산문**에서 나왔다.

- **5 → 6**: R-1 이 §0.4-b 에서 `ReleaseGateMatchesCiGateTest` 의 javadoc 을 고치며 그 문자열을 새로
  적었다. 낡은 산문을 고치는 행위가 그 산문을 세는 grep 의 결과를 바꿨다
- **6 → 5**: **이 커밋이 그 문장을 다시 썼다.** "What this cannot see" 절에서 카빙이 사라지면서 그
  `{@code @Tag("playwright")}` 도 함께 사라졌다

IMPORTANT: 이 표의 첫 판본은 가운데 칸을 **"2026-09-05"** 로 적어 6을 오늘의 값으로 실었다. 그것은
**착수 직전의 값**이었고, 같은 커밋이 그것을 5로 되돌린 뒤였다. 즉 *"다음 사람이 5를 기대하고 6을 보면
멈추게 된다"* 고 적어 둔 표가 **정확히 반대 방향으로** 사람을 멈추게 만들고 있었다 — 6을 기대하고 grep
하면 5가 나온다. 규칙 여섯(*"N건도 도구가 만들어 낸 숫자"*)이 겨누는 실패가 **그 규칙을 인용하는 표
안에서** 재발한 것이고, 빠져 있던 것은 세는 법이 아니라 **세는 시점**이었다. 날짜 하나로는 부족하다 —
같은 날 같은 커밋의 앞뒤가 다르기 때문이다.
...
- `ReleaseGateMatchesCiGateTest` 에 검사 하나(`releaseSkillDoesNotCallAGatedTierUngated`). **리뷰가
  낳은 것**이며 사연은 아래 "산문 다섯" 항목에 있다 — 게이트가 도는 태스크를 릴리스 스킬이 opt-in
  이라고 부르면 실패한다. 기존 `SKILL_GATE_DECLARATION` 정규식을 넓히지 **않았다**: 그 정규식이 느슨한
  산문을 훑지 않는 것은 javadoc 이 이유를 적어 둔 결정이므로(규칙 다섯 — 값이 왜 그 값인지 먼저 찾는다),
  넓히는 대신 파싱이 필요 없는 좁은 불변식을 옆에 세웠다
...
- **낡아 있던 산문 다섯** — 처음에는 "셋" 으로 적었고, 그것이 틀렸다. 전수는 이렇다.

  | # | 자리 | 무엇이라고 적고 있었나 |
  |---|------|----------------------|
  | 1 | `.github/workflows/build.yml` 의 `integration` job 주석 | *"the one tier still running nowhere is `playwrightTest`"* |
  | 2 | `aimon.java-conventions.gradle.kts` 의 계층 주석 | *"the tier that still runs nowhere is `playwrightTest`"* |
  | 3 | `ReleaseGateMatchesCiGateTest` 의 "What this cannot see" | *"One tier runs in neither place"* |
  | 4 | `PlaywrightLifecycleManagerTest:26` 의 javadoc | *"태그가 붙으면 CI 가 회귀를 잡지 못한다"* — 이제 CI 가 잡고, 그때도 그 문장은 실제보다 약했다 |
  | 5 | **`.claude/skills/release/SKILL.md:83`** | *"`playwrightTest` is the only opt-in tier outside both"* — **처음 착수에서 놓쳤고 리뷰가 잡았다** |

  IMPORTANT: 다섯 번째가 이 항목에서 가장 값진 자리다. **같은 파일의 세 줄 위(`:80`)를 이 작업이 직접
  고쳤고**, 그 선언을 지키라고 있는 테스트(`releaseSkillDescribesTheRealGate`)가 **그동안 초록이었다** —
  `SKILL_GATE_DECLARATION` 정규식이 백틱 안의 태스크 목록만 읽으므로 옆줄 산문은 사정거리 밖이기
  때문이다. 즉 이 저장소가 산문 드리프트를 막으려고 만든 장치가 **자기 파일 안에서 세 줄 떨어진 모순을
  못 봤다.** 그래서 고치는 것으로 끝내지 않고 검사를 하나 더 세웠다(위 `한 것`). 그리고 "셋" 이라는 숫자
  자체가 규칙 일곱의 사례다 — 그 숫자는 **커밋 하나가 고친 자리의 수**였는데 항목 전체의 수인 것처럼
  읽혔고, 넷째는 다른 커밋에 있었으며 다섯째는 아예 없었다.
