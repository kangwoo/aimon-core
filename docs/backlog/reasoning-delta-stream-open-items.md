...
서버를 상대로 확인된 적이 없다. **이 문단은 이제 `RD-7` 로 등록되어 있다** — 닫힌 항목의
산문 안에 있으면 색인이 그것을 드러내지 못하기 때문이다.
...
## RD-7 — `response.reasoning_text.delta` 는 어떤 서버에서도 관측된 적이 없다

*(2026-09-10 등록. 출처는 #71 국면의 실측 — RD-2 닫힘 기록 안에 산문으로만 있던 것을 항목으로 올렸다.)*

**무엇을.** 매퍼는 추론 델타 계열을 **둘** 읽는다. 요약 계열(`response.reasoning_summary_text.delta`)은
#71 이 611건을 세어 확인했고, 원문 계열(`response.reasoning_text.delta`)은 **한 번도 온 적이 없다.**
그 분기가 실제로 도착하는 이벤트 이름과 맞는지 확인하거나, 맞지 않으면 고친다.

**왜.** 이름이 틀려도 400 이 나지 않는다 — **빈 채널이 나오고, 그것은 모델이 생각하지 않기로 한 것과
구별되지 않는다.** #71 이 존재한 이유가 그 실패 모드이고, 이 항목은 그 이유가 계열 **하나에 대해서만**
해소되었다는 기록이다. 오늘 영향이 없는 것은 사실이다 — 도달 가능한 모델이 요약 계열만 보내고, 두 계열을
한 게이트 아래 흘리는 결정이 바로 그런 모델을 위한 것이다. 영향이 없다는 것과 확인되었다는 것은 다르다.

**어디.** `OpenAIResponsesStreamingMapper:105-109`(`isReasoningTextDelta` 분기, 2026-09-10). 요약 계열은
바로 위 `:103-104` 이며 그쪽은 실측되었다.

**언제 다시 볼까.** 원문 추론 텍스트를 내보내는 모델에 닿을 수 있게 될 때. 그때까지 이 분기는 **코드로만
참**이고, 그 사실이 적혀 있어야 다음 사람이 "두 계열 다 실측됐다" 로 읽지 않는다.

---

## RD-8 — Responses 경로의 `incomplete` stop-reason 두 갈래가 미측정이다

*(2026-09-10 등록. 출처는 같은 국면 — 그 실행의 `deviations.md` D-5.)*

**무엇을.** `OpenAiResponseStopReasons.fromStatus` 가 `status: "incomplete"` 아래에서
`incomplete_details.reason` 을 `max_output_tokens` 와 `content_filter` 로 가른다. **두 갈래 어느 쪽도
실제 응답으로 확인된 적이 없다** — #71 의 프로브는 전부 정상 종료였다.

**왜.** 매핑이 틀리면 `StopReason.UNKNOWN` 으로 떨어지고, 그것은 **턴이 왜 끝났는지를 호출자에게서
빼앗는다** — 예산이 다해 잘린 응답과 필터에 걸린 응답이 같은 값으로 보인다. Chat Completions 쪽
어휘(`OpenAiStopReasons.fromWire`)는 오래 돌아서 실측된 것과 같으므로, 미확인인 것은 이 엔드포인트의
어휘뿐이다.

**어디.** `OpenAiResponseStopReasons:42-52`(`fromStatus` 의 두 `incompleteReason` 분기, 2026-09-10).

**언제 다시 볼까.** 값이 싸다 — `max_output_tokens` 를 아주 작게 준 요청 하나면 첫 갈래가 발화한다.
...
*(2026-09-10 등록. 출처는 #43 을 닫은 국면 — 그 실행의 `build/summary.md`.)*

**무엇을.** #43 은 `OpenAILlmClient` 의 비자명한 동작들을 지목하며 Responses 경로가 그것을 되돌리지 말라고
적었다 — 중단 레버(`StreamResponse.close()`), 이미 취소된 경우의 빠른 경로, 취소 가능한 비스트리밍 호출을
스트리밍 경로로 태우는 것, `SseException` 을 상태코드가 아니라 페이로드로 분류하는 것,
`RequestOptions` 가 `null` 을 돌려주어 클라이언트 기본값을 건드리지 않는 것. **다섯 다 살아 있고, 근거는
조심스럽게 옮겼다는 것이 아니라 애초에 옮긴 적이 없다는 것이다** — 엔드포인트 차이는
`OpenAIEndpointExchange` 아래에 있고 두 경로가 문자 그대로 같은 코드를 쓴다.

**왜.** 그 논거는 다섯을 각각 포팅한 것보다 **강하지만**, 라이브 호출이 그중 어느 것도 지나가지 않는다.
특히 `SseException` 분기는 **스트림 도중의 서버 실패**를 필요로 하는데, 그것을 인위적으로 만들어 과금하는
일은 하지 않았다. `Closes #43` 을 "#43 전부가 실측되었다" 로 읽으면 안 되는 자리가 여기다.

**어디.** `OpenAIResponsesCancellationTest` · `OpenAIResponsesExceptionRoutingTest` ·
`OpenAIResponsesErrorEventTest` · `OpenAIResponsesRequestTimeoutTest` 가 다섯을 픽스처로 묶는다
(2026-09-10). 없는 것은 그 아래의 실측이다.

**언제 다시 볼까.** 두 경로가 **갈라질 때.** 오늘 이 항목의 위험이 낮은 이유가 "같은 코드" 하나이므로,
Responses 전용 분기가 하나라도 생기는 순간 그 근거가 사라지고 다섯은 각각 확인해야 한다.
