...
그래서 **입력 필드 한 자리만** 저하시킨다: 손상 신호는 그대로 던져야 한다. 남아 있는 선재 동작은
5번으로 등록했다.

IMPORTANT: 이 절의 초판은 그 한정을 *"`catch` 를 넓히지 않고"* 로 적었는데, **그 표현이 곧 다음 정정을
불렀다** — 경계를 예외 타입으로 그으면 값 객체가 던지는 `IllegalArgumentException` 이 그대로 새어 나간다.
§0.5 가 그것이고, 지금의 `catch` 는 실제로 넓다. 경계는 타입이 아니라 **필드**다.
...
| 새 빌드가 **더 새로운** 빌드의 항목을 | 위의 거울상이고, 이 형식이 새로 만든 방향이다. 그 필드를 읽다 나온 것은 **무엇이든** 같은 `asText()` 렌더로 저하하고 WARN 을 남긴다 — 거절하면 배치가 통째로 사라지기 때문이다(§0.4·§0.5) |
...
| `UserInputCodecTest` — 값 객체의 거절도 저하하는가 (§0.5) | 경계가 예외 타입으로 그어진다 | 정규화·넓은 catch 를 둘 다 제거 | 둘 다 FAILED |
...
**정정** — 넷이다. *"인코딩 자체는 이미 있다"* 는 참이지만 재사용 가능하다는 뜻은 아니었고(§0.2),
*"조용히 사라진다"* 는 심각도는 틀렸으며 그것이 닫는 근거까지 끌고 갔고(§0.3), 이 변경이 세운 원칙을
이 변경이 만든 새 경로에 적용하지 않았고(§0.4), 그 처방의 경계를 **예외 타입**으로 그어 절반이 새어
나갔다(§0.5).

**넷째가 늦게 붙은 것 자체가 규칙 일곱의 세 번째 사례다.** §0.5 는 새로 열렸는데 이 줄은 "셋" 인 채
남아 있었고, 이 문서에서 항목이 자기 정정 절을 열거하는 유일한 자리가 여기다 — 즉 §0.5 를 가리키는
문장이 문서 전체에 **없었다**. 앞의 두 사례(§0.4 의 Mongo 행, 색인의 문단)와 같은 모양이고, 셋 다
**본문을 고치면서 그것을 세는 자리를 안 고친 것**이다. 그래서 이 절을 하나 늘릴 때 함께 세야 하는 자리를
못박아 둔다 — 이 "정정" 줄, `## 0.` 아래의 절 목록, 제목의 집계, [`README.md`](README.md) 의 표.
...
~~**처방을 적지 않는 이유**~~ — **처방이 정해졌다** *(2026-09-06)*.
...
같은 문서가 세 백엔드의 배치 소실을 전부 실컨테이너로 재현했고(Redis·Postgres 는 이번이 처음),
`priority` 는 세 백엔드가 디코드 **전에** 필터링하므로 이 결함의 트리거가 아니라는 것도 측정했다.

**그래도 항목은 열려 있다** — 설계는 코드가 아니다. 닫히는 것은 §7 의 계약 스위트가 초록이 되는
시점이며, 규칙 다섯이 요구하는 "고치기 전에 실패하는 테스트" 는 이미 있다(오늘의 트리가 그 빨간
상태다). 남은 처방 하나(dead-letter)는 기각이 아니라 **보류**이고 재검토 트리거는 그 문서 §9 에 있다.
