...
않습니다.** 인박스가 나르는 것은 입력의 렌더가 아니라 입력 자체입니다. 예전에는 그렇지 않았고 — 크로스
노드 제출이 `String` 이었습니다 — 그래서 같은 애플리케이션이 스케일아웃하는 순간 멀티모달이 조용히
사라졌습니다. 요청은 성공하고 턴은 돌지만 모델이 받는 것은 `[Image: image/png, 41231 bytes]` 라는
텍스트뿐이었습니다.
