...
| `thinkingDialect` | which shape this model's thinking request takes — `unknown` · `budgeted` · `adaptive`. **Read by the anthropic branch only.** This is the value `llm.anthropic.thinkingMode: auto` below goes and asks for | `unknown` — the table cannot answer, and the client leaves whatever was configured as it was |
