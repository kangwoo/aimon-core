...
### 어디에 선언하는가 — 네 표면과 그 천장

허용 목록은 네 곳에서 선언하며, 키 이름은 **네 곳 모두 `allowed-tools`** 다.

| 표면 | 마크다운 | 코드 |
|------|----------|------|
| **에이전트**(메인) | `agent.md` | `DefaultAgent.builder().tools(...)` |
| **서브에이전트** | `agents/*.md` | `Subagent.builder().tools(...)` |
| **스킬** | `SKILL.md` | — |
| **커맨드** | 커맨드 파일 | — |

`agent.md` 의 다른 키는 camelCase(`maxIterations`)인데 이 키만 kebab 인 것은 의도다 — Agent Skills 스펙의
이름이고 나머지 세 표면이 이미 그 철자를 쓴다. 대신 `allowedTools` 라고 적으면 **조용히 무시되지 않고 파싱이
실패한다**: 미지의 키는 원래 무시되므로, 무시하면 제한했다고 믿는 채 무제한으로 도는 에이전트가 남는다.

선언한 목록은 **두 지점**에서 작동하며, 둘 다 같은 값 하나를 읽으므로 서로 어긋날 수 없다.

| 지점 | 무엇을 하는가 |
|------|---------------|
| 프롬프트 | 목록에 **이름이 없는** 도구는 LLM 에 보내는 도구 정의에서 빠진다 — 모델이 애초에 고를 수 없다 |
| 실행 | 그래도 이름을 부르면 거부된다. "모르는 도구" 가 아니라 "허용되지 않음" 이며, 레지스트리를 좁히지 않는 이유가 그것이다 |

**패턴 항목은 그 도구를 계속 제안한다.** `Bash(git:*)` 는 `Bash` 를 제안 목록에서 빼지 않는다 — 도구 목록은
어떤 인자가 허용되는지 말할 수 없기 때문이며, 인자 판정은 실행 지점이 한다.

IMPORTANT: **호출자의 목록을 넘지 못한다.** 서브에이전트·워크플로·스킬 포크로 띄운 실행은 물론, 스킬이
자기 ReAct 루프에서 쓰는 도구도 자기 목록과 **호출자의 목록 둘 다**에 묶인다 — 모델이 `Skill` 을 부르든
사용자가 `/my-skill` 을 타이핑하든 같다. 두 목록에 공통 항목이 하나도 없으면 그 실행은 **거부된다**:
빈 목록은 어디서나 "무제한" 으로 읽히므로, 교집합이 비었다고 빈 목록을 넘기면 가장 엄격한 짝이 가장
느슨한 것으로 뒤집힌다.

그리고 **이름만 적은 항목은 같은 이름의 패턴 항목이 옆에 있으면 "무제한" 이 아니다.** `Read` 와
`Read(/tmp/**)` 를 함께 적으면 그 목록은 `/tmp` 만 뜻하고, 교집합도 그 패턴을 살려서 내려보낸다 — 그러지
않으면 포크가 자기를 띄운 실행보다 넓어진다.

마지막으로 **도구가 하나도 남지 않으면 경고를 남긴다.** 오타 난 이름, 부수효과 천장 위의 도구만 적은 목록,
deferred 도구 배포에서 `ToolSearch` 를 빠뜨린 목록이 그렇게 된다. 어떤 프로바이더도 빈 `tools` 필드를 거부하지
않으므로 모델은 산문으로 답하고 실행은 깨끗하게 끝난다 — 하지 않은 일이 된 일처럼 보인다.
