...
**Sealed session-log segments** are a fifth, optional store. On a version-2 session log, ranges the LLM view no longer shows verbatim are moved out of the record into a `SessionLogSegmentStore` (`at.aimon.core.agent.session.store`), and the record's manifest names them. Pass the same instance to the transcript manager (through `SessionLogStorage`) and to `SessionRouterBuilder.sessionLogSegmentStore(...)`, so a session delete removes its segments after the record. Keep it in the same database as the records — a segment is data moved out of a record: `MongoSessionLogSegmentStore` (collection `session_log_segments`, see `init.js`), `PostgresSessionLogSegmentStore` (table `session_log_segment`, applied with `V2__session_log_segment.sql` after `V1__init.sql`) or `RedisSessionLogSegmentStore`. Deletes go through `SessionStore.segments(...)`, the fenced view; writes are not fenced, because a segment no manifest names is never read. Design: `docs/design/session/session-log.md` §5.
