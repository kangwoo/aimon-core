...
| ~~**F-4**~~ | ~~**Forwarding thinking text to `LlmStreamSink`** as a user-visible "thinking" stream.~~ | **DONE** — [`reasoning-delta-stream.md`](reasoning-delta-stream.md) (#62), together with F-7 below, because taken separately the first ships a channel that is always empty. `LlmStreamChunk.Kind.REASONING_DELTA` carries the text in a field of its own and `AssistantReasoningDelta` carries it out as a sixteenth `AgentExecutionEvent` subtype. The forwarding is gated on **configuration** rather than on the arrival of the deltas, which the design of record did not anticipate: on the budgeted dialect `thinking_delta` events arrive today and are swallowed, so "forward whatever arrives" would have turned a user-visible reasoning stream on for every existing `thinkingMode: extended` deployment without it asking. |
...
| ~~**F-7**~~ | ~~**`display: "summarized"` / `"updates"`.**~~ **This row named the wrong second value and is the origin of a defect** — see the correction at the end of this cell. | **DONE** — [`reasoning-delta-stream.md`](reasoning-delta-stream.md) (#62). Shipped with F-4 rather than after it: "costs tokens for text nothing renders yet" stopped being true the moment there was something to render it. Reached from both configuration surfaces as `llm.anthropic.thinkingDisplay` / `aimon.llm.anthropic.thinking-display`, opt-in and unset by default. **A11's *"moot on this SDK before it is declined"* needs a correction:** the SDK still has no `display` field — `ThinkingConfigAdaptive` in `anthropic-java-core` 2.13.0 carries `type` and nothing else — and the round got past that with `putAdditionalProperty`, the SDK's own `@JsonAnySetter` escape hatch and the one this repository already uses to write unmodelled request fields. Written on the **adaptive** shape only: whether the budgeted shape accepts a `display` sibling is unmeasured, and on that dialect the text already arrives, so nothing is sent there and the operator who writes the key under `EXTENDED` is told once that the word reached nothing while the forwarding did. **CORRECTION (2026-09-10, measured): the two values this row names are not the two the server accepts.** A live probe returns 400 for `display: "updates"` with the body `thinking.adaptive.display: Input should be 'summarized', 'omitted'`, identical to the body a bogus control value gets — so the field is validated and the accepted set is exactly `{summarized, omitted}`. This row is where the wrong name came from, and **`:165` of this same file had already recorded `omitted` as the default**, so the document contradicted itself. `AnthropicThinkingDisplay` shipped `UPDATES("updates")` on the strength of this row; that constant is dropped rather than renamed to `OMITTED`, for the reasons in [`reasoning-delta-stream.md`](reasoning-delta-stream.md) §12.2. |
