...
> **2026-09-10 (#69) — 이 절이 답하지 않는 것을 적어 둔다: *어느 이름*이 아니라 *어느 플래그*가 살아남는가.**
> 위 표는 선언이 어느 이름에 대해 답하는지를 못박고, 그 이름의 행에서 **적지 않은 플래그가 어떻게 되는지는
> 한 줄도 말하지 않는다.** 실제 규칙은 `withDefaultsExtendedBy` 의 javadoc 에만 있다 — *"선언은 그 이름에
> 대한 행 전체이지 행에 대한 패치가 아니다"*. 즉 내장 표에 있는 이름을 선언하면 **적지 않은 플래그는 그 행이
> 말하던 값이 아니라 fail-open 값으로 떨어진다.**
>
> 이 침묵이 대가를 치렀다. 운영자를 마주 보는 문서 **셋**(CLI 레퍼런스 · 스타터 가이드 ·
> `default-config.yaml`)이 *"적지 않은 것은 오늘의 동작을 그대로 유지한다"* 고 적고 있었고, 그것은 **내장
> 표가 모르는 이름에 대해서만** 참이다. 설계 기록이 그 규칙을 어디에도 적지 않았으므로 세 문서가 코어와
> 반대되는 말을 하는 동안 아무도 대조할 것을 갖지 못했다.
>
> **#69 가 그것을 급한 문제로 만든다.** 지금까지 이 표면의 문서화된 대상은 *내장 표가 들어 본 적 없는
> 게이트웨이 이름*이었다 — 덮을 행이 없으니 전체 교체에 비용이 없다. `thinkingDialect` 는 문서화된 대상이
> **언제나 행을 가진 이름**(`claude-*`)인 첫 키이고, 그 행의 다른 플래그는 **억제**(`supportsSamplingParameters
> (false)`)다. 그래서 `{claude-sonnet-5: {thinkingDialect: unknown}}` 은 억제를 되돌려 주고
> `temperature` 를 400 을 내는 모델로 보낸다 — **경고도 없이**, 억제 WARN 은 플래그가 `false` 일 때만 울리기
> 때문이다.
>
> **결정은 바뀌지 않았다**(교체이지 병합이 아니다 — 그것은 #46 의 설계이고 되열려면 그 라운드가 필요하다).
> 이 라운드가 한 것은 코어가 이미 말하던 것을 설정 표면 쪽 문서 셋이 함께 말하게 만든 것, 그리고 세 모듈에
> 그 동작을 못박는 테스트를 둔 것이다. 남은 일반형 — 선언이 내장 행을 덮으면서 그 행이 말하던 플래그를
> 적지 않았을 때 기동 시 경고 — 은
> [`../../backlog/llm-config-surface-open-items.md`](../../backlog/llm-config-surface-open-items.md) 의
> **L-8** 이다. 네임스페이스 판정은 §2.7 의 2026-09-10 블록에 있다.
...
> **2026-09-10 (#69·#72) — 기준의 세 번째 적용, 그리고 기준이 무엇을 고르는지에 대한 정련.**
> 이 라운드가 키 둘을 더한다. 둘 다 `model-capabilities.<model>` 맵 **안의 잎**이고, 둘 다 공유
> 네임스페이스에 남는다.
>
> | 키 | CLI | 스타터 | 판정 |
> |---|---|---|---|
> | `thinkingDialect` (#69) | `llm.modelCapabilities.<m>.thinkingDialect` | `aimon.llm.model-capabilities.<m>.thinking-dialect` | 공유 |
> | `supportsReasoningSummary` (#72) | `llm.modelCapabilities.<m>.supportsReasoningSummary` | `aimon.llm.model-capabilities.<m>.supports-reasoning-summary` | 공유 |
>
> **첫 번째가 논쟁적이다.** 이슈 #69 의 본문은 이 기준이 이 키를 **벤더 네임스페이스로** 보낸다고 읽는다 —
> "thinking" 은 Anthropic 의 단어이고, #54 가 `thinkingMode` 에 대해 정확히 그 독법으로 세 키를
> `llm.anthropic.*` 로 내렸기 때문이다. 그 독법을 저울에 올렸고, 졌다. 근거 셋:
>
> 1. **기준의 주어는 키 *가족*이지 잎이 아니다.** 지금까지의 세 적용은 전부 *네임스페이스가 어디서
>    시작하는가*를 골랐다(`aimon.llm.model-capabilities.*`, `aimon.llm.anthropic.*`,
>    `aimon.llm.reasoning-effort`). 여기 후보는 홀로 선 `thinkingDialect` 가 아니라
>    `model-capabilities.<model>.thinkingDialect` — 배치가 이미 정해진 맵 안의 플래그 하나다. 잎에만
>    기준을 적용하면 **레코드 타입 하나가 두 네임스페이스로 쪼개진다**: 개명된 게이트웨이 모델 하나를
>    서술하는 운영자가 일곱 플래그를 한 곳에, 여덟 번째를 다른 곳에, 양쪽 다 모델 이름을 키로 적게 된다.
>    그것이 `ThinkingDialect` 의 javadoc 이 피하겠다고 이름 붙인 상태다.
> 2. **잎 이름은 이 라운드가 내리는 결정이 아니다.** §2.6 이 이 잎 이름들을 `ModelCapabilities` 자바 필드
>    이름의 **글자 그대로의 전사**로 못박았고 `supports` 접두어를 떼는 것조차 거절했다. 테스트 1 은
>    *우리가 고른 이름*이 벤더 개념을 담는지를 묻는데, 여기서는 아무도 고르지 않았다.
> 3. **#54 가 이 키를 이미 여기에 배치했다, 글로.** 그 문서 §12(범위 밖)가
>    `thinkingDialect` 의 여섯 번째 `model-capabilities` 키에 대해 *"여기 속하지 않는다 … `thinking*` 키는
>    **이 배포가 무엇을 원하는가**를, `thinking-dialect` capability 키는 **모델이 무엇인가**를 말하며,
>    다섯 플래그의 검증·우선순위·대소문자 규칙을 물려받는 공유 네임스페이스의 모델별 맵에 놓인다"* 라고
>    적었고, 트리거까지 이름 붙였다 — *"게이트웨이 뒤에서 `auto` 가 동작해야 하는 개명된 Claude 모델을 가진
>    운영자"*, 즉 #69 다. 새 판단이 #54 와 모순되는 것이 아니라 **#54 자신의 판단이 그것이 이름 붙인
>    트리거에서 집행되는 것**이다.
>
> 테스트 2(뜻이 벤더마다 다른가)도 아니오다 — *"이 모델의 thinking 요청 파라미터는 어느 모양을 취하는가"*
> 는 배타적인 두 모양을 가진 어느 벤더든 답할 물음이고, 오늘 그런 벤더가 하나뿐이라는 것은 §2.7 자신이
> 명시한 **비-기준**이다.
>
> `supportsReasoningSummary` 는 둘 다 아니오라 논쟁이 없다 — 이름은 중립 SPI 필드의 전사이고, 물음
> ("이 모델이 추론 요약 요청을 받는가")은 Anthropic 이 `thinking.display: summarized` 로 답하는 같은 축이다.
> 오늘 소비자는 OpenAI Responses 경로 하나뿐이며, 그것은 §2.7 이 쪼개는 이유가 아니라고 적어 둔 바로 그
> 사실이다.
>
> **B-21 은 건드리지 않는다.** #54 §3.5 가 *분할* 결정이 그리로 간다고 세웠고, 이것은 **이미 공유인 가족
> 안의 잎을 쪼개지 않기로 한** 결정이므로 기준의 기록이 사는 이 절에 남는다.
>
> 이로써 선언 표면은 **여덟 키**이고 **여덟 전부가 양쪽 표면에 바인딩된다.** 그것이 #69 의 전부다 —
