...
> **이 문단의 앞 절반은 더 이상 사실이 아니다** *(2026-09-05)*. `SubmitRequest` 는 `UserInput` 을 나르고
> 인박스 와이어가 그 인코딩을 싣는다 — 무엇을 어떻게 했는지는
> [`backlog/interrupt-open-items.md` §2](../../backlog/interrupt-open-items.md). 아래 문단은 **설계 시점의
> 기록**으로 그대로 두고(§14 의 IMPORTANT 와 같은 규칙), 뒤 절반 — 라우터가 재시도를 노출하지 않는다 —
> 은 지금도 참이다.
