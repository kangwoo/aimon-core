...
넘겨받기가 **실패해도 그 메시지는 그대로 실행한다.** 실패는 셋 중 하나이고 모두 "가져가면 안 되는" 경우이지
"돌리면 안 되는" 경우가 아니다 — 패스를 연 제출 자신의 예약(이미 이 노드 이름이 적혀 있다), `DONE`(누군가의
캐시된 답), 만료된 예약(그 forward 는 이미 포기했다). 메시지는 이미 at-most-once inbox 밖이라 거절하면 아무
노드도 복구할 수 없고, 잃는 것은 빠른 감지뿐이며 그것은 이 연산이 없던 때의 상태와 같다.
...
하나만 이긴다. 그 결과 **이름이 비어 있는 구간과 아무도 돌리지 않는 구간이 정확히 일치**하며, 그것이
`findStaleInFlight` 가 홀더 없는 항목을 건너뛰어도 커버리지를 잃지 않는 근거다(§6.3 D).
