...
턴의 키를 리셋**해 같은 요청이 두 번 실행됐을 것이다. 슬롯은 쌍(키 + reserverId)을 원자적으로 스왑한다 —
따로 두면 틱이 턴 5 의 키를 턴 4 의 reserver 로 읽을 수 있고, `touch` 는 홀더가 어긋나면 **조용히 무시**한다.
...
| **수집된 뒤** 홀더가 죽은 전달 턴 | 스위퍼는 **홀더 이름이 붙어 있는 예약만** 본다. forward 는 `releaseHolder` 로 홀더를 비워 inbox 에 넘기므로(§7.1 F2), 드레인한 노드가 그 메시지를 돌리다 죽으면 예약은 홀더 없는 `IN_FLIGHT` 로 남아 `findStaleInFlight` 에 걸리지 않는다 — 호출자는 `HOLDER_LOST` 가 아니라 forward TTL(5분) 타임아웃으로 답을 받는다. 재-초인종은 여기에 쓸모가 없다(메시지가 이미 inbox 밖이다). 고치려면 드레인이 그 예약의 홀더를 넘겨받아야 하고, `IdempotencyStore` 에 그럴 수 있는 연산이 없으므로 SPI + 백엔드 4종이 함께 움직인다 |
