...
대해 볼 수 없다** — 호출자는 forward 마감으로 되돌아가며, 그것이 이 연산이 없던 때의 상태다. 그리고 (2)
**거절된 경우에만** 그 결과를 멱등 캐시에 쓰지 않는다. `markDone` 은 네 백엔드 모두 키만 보고 매치하므로, 방금
"남의 것"이라고 판정한 항목을 덮어쓰는 것이 안전이 아니라 해악이기 때문이다(§9.2). 호출자는 어느 쪽이든 rail 로
답을 받는다.
...
`markDone` 보다 엄격히 나쁘다(시도는 공짜다). 그래서 넘겨받기는 **승리 / 거절 / 알 수 없음**을 구분해서 돌려주고,
"알 수 없음" 은 이 연산이 없던 때의 동작을 그대로 유지한다.
