...
이 네 단어는 `docs/backlog/README.md` 가 **서로 다른 뜻으로 정의한 것**이라 바꿔 쓸 수 없다. 앞의 셋은
상태이고 색인에 저마다 칸이 있다. `접힘` 은 상태가 아니라 열림의 한 종류다.
...
| 접힘 | folded | 의식적으로 접어 둔 열린 항목. 열림으로 세고, 색인에 칸을 따로 두지 않는다 |
