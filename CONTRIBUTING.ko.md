...
실행은 API 를 호출하고 청구됩니다. 그리고 빌드하는 변경과 무관한
...
나머지 둘은 번역에 관한 것이고 `translations` 잡에서 함께 돕니다. 각각 무엇에 실패하고 그중 하나는
왜 대체로 실패하지 않는지는 [번역](#번역) 절에 있습니다. 둘 다 전체 git 이력이 필요하므로 얕은
