...
/**
 * Integration tests for {@link RedisIdempotencyStore} against a real Redis container.
 */
@DisplayName("RedisIdempotencyStore integration")
@Tag("docker")
class RedisIdempotencyStoreIntegrationTest {
    ...
    void setUp() {
        ...
        store = new RedisIdempotencyStore(connection);
    }
    ...
    void findStaleScansAndFilters() {
        ...
        final var stale = store.findStaleInFlight(now.minusSeconds(60));
    ...
    void findStaleSkipsReleasedReservations() {
        ...
        final var stale = store.findStaleInFlight(now.plusSeconds(600));
    ...
    void discardReservationFreesAQueuedReservation() {
        ...
        // releaseHolder is the only route to a "holderId":null entry, and it is the state this method exists for: the
    ...
    void discardReservationSparesADoneEntry() {
        ...
        // markDone drops the holder as well, so the stored JSON carries "holderId":null exactly like a reservation
        // does — only the status match tells them apart, and a false return alone would not prove the value survived.
    ...
    void discardReservationSparesAHeldEntry() {
        ...
        // A turn is executing on h-1 right now: the key must survive and stay touchable by its holder.
