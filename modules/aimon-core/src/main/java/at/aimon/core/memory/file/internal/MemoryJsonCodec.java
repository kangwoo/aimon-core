...
package at.aimon.core.memory.file.internal;
...
public final class MemoryJsonCodec {
    ...
    /**
     * The mapper reads and writes {@link JsonNode} trees only — every domain
     * field, timestamps included, is mapped by hand below
     * ({@code Instant#toString()} out, {@code Instant#parse} back). No POJO
     * binding happens here, so no {@code JavaTimeModule} is registered: it was
     * inert while this code lived in {@code aimon-memory-file}, and registering
     * it would have made {@code jackson-datatype-jsr310} a published dependency
     * of {@code aimon-core} for nothing. The on-disk format is unchanged.
     */
