 ...
 *
 * <ul>
 * <li><b>No messages.</b> Either the execution added nothing, or — in the version-1 write mode only — its history was
 * rewritten underneath it and no delta could be taken. Both mean "nothing to say"; see
 ...
 *
 * <p>
 * An execution's delta is sent in chunks of at most {@code maxIngestTokens}, cut at legal cuts only — see
 * {@link IngestChunks}. With an append-only log a compacted execution is ingested as it was said, and nothing bounds
 * that to one window any more. Each chunk is one {@link MemoryIngestor#ingest} call; the delta the executor hands over
 * already leaves out what the runtime injected.
 *
 * <p>
...
public final class IngestingExecutionMemorySink implements ExecutionMemorySink {
    ...
    private final int maxIngestTokens;
    ...
    public IngestingExecutionMemorySink(MemoryIngestor ingestor, Workspace workspace, MemoryPeerResolver peerResolver) {
        this(ingestor, workspace, peerResolver, IngestChunks.DEFAULT_MAX_INGEST_TOKENS, new HeuristicTokenEstimator());
    }

    /**
     * Creates a sink with an explicit chunk budget.
     *
     * @param ingestor
     *            the tier messages are fed to — already wrapped for redaction by the assembly (must not be null)
     * @param workspace
     ...
     * @param peerResolver
     *            decides whose memory an execution is attributed to (must not be null)
     * @param maxIngestTokens
     *            the estimated size of one ingest call; {@code 0} or less sends the delta in one call
     * @param tokenEstimator
     *            sizes the messages (must not be null)
     */
    public IngestingExecutionMemorySink(MemoryIngestor ingestor, Workspace workspace, MemoryPeerResolver peerResolver,
            int maxIngestTokens, TokenEstimator tokenEstimator) {
        ...
        this.maxIngestTokens = maxIngestTokens;
    ...
    public void afterExecution(ExecutionMemoryUpdate update) {
        ...
        final List<List<Message>> chunks = IngestChunks.split(update.getMessages(), maxIngestTokens, tokenEstimator);
            ...
            try {
                final MemoryIngestReceipt receipt = ingestor
                        .ingest(MemoryIngestRequest.builder().observer(PeerView.of(workspace, peer.get()))
                                .sessionId(sessionId.get().value()).messages(chunk).build());
                log.debug("Memory ingest: session={} chunk of {} accepted={} derived={}", sessionId.get().value(),
                        chunk.size(), receipt.getAccepted(), receipt.isDerived());
            } catch (RuntimeException e) {
                // Fire-and-forget: the execution has already produced its answer, and losing it because a memory
                // backend is unreachable would be the wrong trade in every deployment. The remaining chunks are
                // still offered — each is a whole part of the conversation on its own.
